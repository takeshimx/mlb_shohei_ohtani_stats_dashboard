...
def all_pitchers_stats():
    ...
    for j in pitch_type_list_all:
        # metrics
        avg_release_speed = pitching_data_all_22[pitching_data_all_22['pitch_name'] == j]['release_speed'].agg('mean')
        pitch_counts = len(pitching_data_all_22[pitching_data_all_22['pitch_name'] == j])
        pitch_pct = len(pitching_data_all_22.loc[pitching_data_all_22['pitch_name'] == j]) / len(pitching_data_all_22)
        # RPM: revolutions per minute
        avg_release_spin_rate = pitching_data_all_22[pitching_data_all_22['pitch_name'] == j]['release_spin_rate'].agg('mean')
        max_release_spin_rate = pitching_data_all_22[pitching_data_all_22['pitch_name'] == j]['release_spin_rate'].max()
        min_release_spin_rate = pitching_data_all_22[pitching_data_all_22['pitch_name'] == j]['release_spin_rate'].min()
...
def individual_pitcher_stats(playerId):
    ...
    for i in pitch_type_list_all:
        # metrics
        avg_release_speed = player_df[player_df['pitch_name'] == i]['release_speed'].agg('mean')
        pitch_counts = len(player_df[player_df['pitch_name'] == i])
        pitch_pct = len(player_df.loc[player_df['pitch_name'] == i]) / len(player_df)
        # RPM: revolutions per minute
        avg_release_spin_rate = player_df[player_df['pitch_name'] == i]['release_spin_rate'].agg('mean')
        max_release_spin_rate = player_df[player_df['pitch_name'] == i]['release_spin_rate'].max()
        min_release_spin_rate = player_df[player_df['pitch_name'] == i]['release_spin_rate'].min()
    ...
    pitch_stats_df_player['Name'] = list(pitching_data_all_22.loc[pitching_data_all_22['pitcher'] == playerId]['player_name'])[0]
...
def individual_pitchers_all_stats():
    ...
    for player in list(pitching_data_all_22['pitcher'].unique()):
        pitchers_all.append(individual_pitcher_stats(player))
...
def all_pitchers_stats_b():
    ...
    total_swings_to_balls = pitchers_all_df.groupby('Pitch type').sum()['Total swings to balls'].reset_index()
    # number of pitchers per pitch type
    num_of_pitchers_tstb = pitchers_all_df.groupby('Pitch type').count()['Name'].reset_index()
    tstb_merged = total_swings_to_balls.merge(num_of_pitchers_tstb, on='Pitch type', how='left').rename(columns={'Name': 'Num of pitchers'})
    # new column for the new stat
    tstb_merged['Average total swings to balls'] = round(tstb_merged['Total swings to balls'] / tstb_merged['Num of pitchers'], 1)

    # merge it with pitch_stats_df_all
    pitch_stats_df_all = all_pitchers_stats()
    pitch_stats_df_all_b = pitch_stats_df_all.merge(tstb_merged[['Pitch type', 'Num of pitchers', 'Average total swings to balls']], on='Pitch type', how='left')
...
def individual_pitcher_stats_v2(df, playerId, startSeason, endSeason):
    ...
    for year in years:
        ...
        pitch_stats_df_player['MLB ID'] = playerId
        pitch_stats_df_player['Name'] = list(df.loc[df['pitcher'] == playerId]['player_name'])[0]
...
def stats_rank_pitcher(pitcher_df, metric, year):
    pitcher_df = pitcher_df.loc[(pitcher_df['Season'] == year) & (pitcher_df['IP'] >= 100)]\
...
def stats_delta_pitcher(pitcher_df, player, metric, year):
    pitcher_df = pitcher_df.loc[(pitcher_df['Name'] == player) & (pitcher_df['IP'] >= 100)]\
                    .sort_values('Season', ascending=True)
    
    # if stat for a give season exists
    if year in list(pitcher_df['Season'].unique()):
        current_year_stat = pitcher_df.loc[pitcher_df['Season'] == year][metric].values[0]
    else:
        return 'No data found'
    
    if year-1 in list(pitcher_df['Season'].unique()):
        prev_year_stat = pitcher_df.loc[pitcher_df['Season'] == year-1][metric].values[0]
    else:
        return 'No data found'

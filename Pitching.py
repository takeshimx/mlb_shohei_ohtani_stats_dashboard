...
pitching_data_sho_all_file = 'pitching_data_sho_all.csv'
...
pitching_data_all_1522_reg_file = 'pitching_data_all_1522_df_reg.csv'
pitching_data_all_1522_reg = load_file(pitching_data_all_1522_reg_file)
pitching_data_all_1522_reg['game_year'] = pitching_data_all_1522_reg['game_year'].astype(int)
...
default_idx = list(pitching_data_all_1522_reg['player_name'].unique()).index('Ohtani, Shohei')
...
pitching_data_all_1522_reg_player = pitching_data_all_1522_reg.loc[pitching_data_all_1522_reg['player_name'] == pitcher]
...
level = st.sidebar.selectbox(
    'Select level:',
    options = ['Season level', 'Date level']
)
...
if level == 'Date level':
    game_date_latest = pitching_data_all_1522_reg_player\
        .loc[pitching_data_all_1522_reg_player['game_year'] == pitching_data_all_1522_reg_player['game_year'].max()]\
            ['game_date'].unique()
...
# --------------------- MAINPAGE ---------------------
st.title(':baseball: Shohei Ohtani Pitching Stats in MLB Dashboard v1.0')
...
with stats_col1:
    ...
    with col1:
        ...
        # Filter 1: a pitcher
        default_idx = list(season_pitcher_stats_all['Name'].unique()).index('Shohei Ohtani')
        ...
        # main data frame is filtered out with pitcher who is selected above
        season_pitcher_stats_all_player = season_pitcher_stats_all.loc[season_pitcher_stats_all['Name'] == stats_pitcher]

        # Filter 2: season(s)
        game_season_latest = season_pitcher_stats_all_player['Season'].unique().max()
...
with left_column_1:
    st.metric(label=':orange[ERA]', value=round(era_sho, 2), delta=round(era_delta_sho, 2),
...
with left_column_2:
    st.metric(label=':orange[ERA+]', value=int(eraplus_sho), delta=int(eraplus_delta_sho),
...
with middle_column_1:
    st.metric(label=':orange[WIN]', value=win_sho, delta=int(win_delta_sho),
              help='Number of wins')
with middle_column_2:
    st.metric(label=':orange[FIP]', value=round(fip_sho, 2), delta=round(fip_delta_sho, 2),
...
with right_column_1:
    st.metric(label=':orange[K/9]', value=round(k9_sho, 2), delta=round(k9_delta_sho, 2),
...
with right_column_2:
    st.metric(label=':orange[bWAR Pitcher]', value=pitch_bwar_sho, delta=round(pitch_bwar_delta_sho, 2),
...
with extra_column_1:
    st.metric(label=':orange[WHIP]', value=round(whip_sho, 2), delta=round(whip_delta_sho, 2),
              help='Walks And Hits Per Inning Pitched')
with extra_column_2:
    st.metric(label=':orange[IP]', value=ip_sho, delta=round(ip_delta_sho, 1),
              help='Inning pitched')
with extra_column_3:
    st.metric(label=':orange[SO]', value=so_sho_b, delta=int(so_delta_sho),
...
with stats_middle_col:
    st.metric(label=':orange[WHIFF%]', value=round(whiff_pct_sho*100, 2),\
...
with left_column: 
    cf.fig_pitch_stats(pitch_dstribution_sho, 'pfx_x', 'pfx_z', pitcher, season, 'Pitch Movement')

# Chart 2: pitch position distribution
with right_column:
    cf.fig_pitch_stats(pitch_dstribution_sho, 'plate_x', 'plate_z', pitcher, season, 'Pitch Position')
...
with left_column_r3:
    ...
    df_selection = individual_stats_all_sho.query(
        'Year == @year')
...
with right_column_r3:
    ...
    default_idx_2 = list(pitchers_stats_1522_all['Name'].unique()).index('Cease, Dylan')
...
with extra_col_1:
    ...
    pitching_stats_against_1822_sho = pd.read_csv('pitching_stats_against_1822_sho.csv')
    # data for all eligible pitchers 2015 ~ 2022
    pitching_stats_against_1522_all = pd.read_csv('pitching_stats_against_1522_all.csv')
    # pitching_stats_against_1822_sho = pitching_stats_against_1522_all.loc[pitching_stats_against_1522_all['pitcher'] == 'Ohtani, Shohei']
    ...
    baa_by_inning_mlb_mean_1522 = pd.read_csv('baa_by_inning_all_1522_reg.csv')
    ...
    default_idx_2 = list(pitching_stats_against_1522_all['year'].unique()).index(2022)
    season_all = col3.selectbox(
        'Select Season for other pitcher',
        pitching_stats_against_1522_all['year'].unique(),
        index=default_idx_2
    )
    default_idx_3 = list(pitching_stats_against_1522_all['pitcher'].unique()).index('Cease, Dylan')
    ...
    # df for selection
    df_selection = pitching_stats_against_1822_sho.query(
        'year == @season_sho'
    )
...
with left_column_r5:
    # Chart 6: ERA+ ranking
    cf.stat_ranking(df=df_selection_comparison,
                    x='Name',
                    y='ERA+',
                    league='MLB',
...
with right_column_r5:
    # Chart 7: ERA+ vs. WAR
    cf.fig_war_comparison(df=df_selection_comparison,
                          x='bWAR_pitcher',
                          y='ERA', sortValue='bWAR_pitcher',
                          topNumber=20,
                          league='MLB',
                          color='bWAR_pitcher',
                          size='bWAR_pitcher',

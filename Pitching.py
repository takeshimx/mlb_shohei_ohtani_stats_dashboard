...
st.set_page_config(
    page_title="Shohei Ohtani Stats Dashboard v1.0",
...
pitching_data_sho_all = query.create_df("SELECT * FROM pitching_data_sho_all;")
...
pitching_data_all_1522_reg_file = "./data/large_data/pitching_data_all_1522_df_reg.csv"
...
level = st.sidebar.selectbox("Select level:", options=["Season level", "Date level"])
...
with stats_col1:
    ...
    with col1:
        ...
        # )
        season_pitcher_stats_all = query.create_df(
            "SELECT * FROM season_pitchers_stats_all_br;"
        )
...
with right_column:
    cf.fig_pitch_stats(
        pitch_dstribution_sho, "plate_x", "plate_z", pitcher, season, "Pitch Position"
    )
...
with right_column_r3:
    ...
    # )
    pitchers_stats_1522_all = query.create_df(
        "SELECT * FROM pitch_stats_all_players_1522_df;"
    )
    ...
    pitcher_comparison = col2.selectbox(
        "Select Pitcher:", pitchers_stats_1522_all["Name"].unique(), index=default_idx_2
    )
...
with extra_col_1:
    ...
    # )
    pitching_stats_against_1822_sho = query.create_df(
        "SELECT * FROM pitching_stats_against_1822_sho;"
    )
    ...
    # )
    pitching_stats_against_1522_all = query.create_df(
        "SELECT * FROM pitching_stats_against_1522_all;"
    )
    ...
    # )
    baa_by_inning_mlb_mean_1522 = query.create_df(
        "SELECT * FROM baa_by_inning_all_1522_reg;"
    )

...
# ---------------------- MAINPAGE ----------------------
st.title(':baseball: Shohei Ohtani Batting Stats in MLB Dashboard v1.0')
# ------------ Key Batting Stats ------------
...
with stats_col1:
    ...
    with col1:
        ...
        # Filter 1: a batter
        default_idx = list(season_batter_stats_all['Name'].unique()).index('Shohei Ohtani')
        ...
        season_batter_stats_all_player = season_batter_stats_all.loc[season_batter_stats_all['Name'] == stats_batter]
...
with col_1:
    st.metric(label=':orange[BA]', value=ba_sho, delta=ba_delta_sho,
              help='Batting average = number of hits / at-bats')
with col_2:
    st.metric(label=':orange[OBP]', value=obp_sho, delta=obp_delta_sho,
...
with col_3:
    st.metric(label=':orange[SLG]', value=slg_sho, delta=slg_delta_sho,
              help='Slugging Percentage represents the total number of bases a player records per at-bat. Formula: (1B + 2Bx2 + 3Bx3 + HRx4)/AB')
with col_4:
    st.metric(label=':orange[OPS]', value=ops_sho, delta=ops_delta_sho,
...
with col_5:
    st.metric(label=':orange[OPS+]', value=ops_plus_sho, delta=opsplus_delta_sho,
...
with col_6:
    st.metric(label=':orange[wRC+]', value=wrc_plus_sho, delta=wrcplus_delta_sho,
...
with col_8:
    st.metric(label=':orange[RBI]', value=rbi_sho, delta=rbi_delta_sho,
              help='Runs Batted In')
with col_9:
    st.metric(label=':orange[bWAR Batter]', value=batting_bwar_sho, delta=batting_bwar_delta_sho,
...
ohtani_stats_batter_1822_file = 'ohtani_stats_batter_1822.csv'
...
all_batters_stats_1522_masterdf_file = 'batted_resutls_statcast_all_1522_reg.csv'
all_batters_stats_1522_masterdf = load_file(all_batters_stats_1522_masterdf_file)

# Filter 1: a batter
default_idx = list(all_batters_stats_1522_masterdf['player_name'].unique()).index('Ohtani, Shohei')
...
batting_data_all_1522_reg_player = all_batters_stats_1522_masterdf.loc[all_batters_stats_1522_masterdf['player_name'] == batter]
...
with left_column:
    cf.fig_batting_speed_angle(df=df_selection,
...
with extra_col1:
    cf.fig_hist_speed_angle(df=df_selection,
                            year=batting_event_season,
                            x='launch_speed',
...
with mid_col_r2:
    ...
    ba_at_strike_count_sho = pd.read_csv('ba_at_strike_count_sho.csv')

    # Filter 1
    batting_season_sc = col1.selectbox(
        'Select Season:',
        ba_at_strike_count_sho['year'].unique(),
        key='batting_year'
    )

    df_selection_sc = ba_at_strike_count_sho.query(
        'year == @batting_season_sc')
    
    cf.fig_ba_hr(df=df_selection_sc,
...
with mid_col2_r2:
    ...
    ba_at_strike_count_all_1522 = pd.read_csv('ba_at_strike_count_all_1522.csv')

    # Filter 1
    batting_season_sc_all = col1.selectbox(
        'Select Season:',
        ba_at_strike_count_all_1522['year'].unique()
    )

    player_name_sc_all = col2.selectbox(
        'Select Player:',
        ba_at_strike_count_all_1522['player'].unique()
    )
    ...
    cf.fig_ba_hr(df=df_selection_sc_all,
...
with mid_col_r4:
    ...
    stats_at_risp_sho_1822 = pd.read_csv('stats_at_risp_sho_1822.csv')
    ...
    cf.fig_stats_at_risp(df=stats_at_risp_sho_1822,
                         playerSelectionVar='Shohei Ohtani')
...
with mid_col2_r4:
    ...
    stats_at_risp_all_1522 = pd.read_csv('stats_at_risp_all_1522.csv')
    ...
    player_name_risp_all = col1.selectbox(
        'Select Player:',
        stats_at_risp_all_1522['player'].unique(),
        index=default_idx
    )
    ...
    cf.fig_stats_at_risp(df=df_selection_sc_all,
                         playerSelectionVar=player_name_risp_all)
